...
s2c_2h <- read_tsv("ali_meta.txt") %>% mutate(treatment = factor(treatment, levels = c("NS", "Form"))) %>% filter(time == "2h")
s2c_6h <- read_tsv("ali_meta.txt") %>% mutate(treatment = factor(treatment, levels = c("NS", "Form"))) %>% filter(time == "6h")
...
#save sleuth objects
saveRDS(so_2h, "objects/ali_2h.rds")
...
dea_2h <- sleuth_results(so_2h, "treatmentForm", "wt") %>%
  ...
  mutate(celltype = "ALI", time = "2h", treatment = "Form", .before = 2)

dea_6h <- sleuth_results(so_6h, "treatmentForm", "wt") %>%
  select(Gene = target_id, fold = b, FDR = qval) %>%
  mutate(fold = fold / log(2)) %>%
  mutate(celltype = "ALI", time = "6h", treatment = "Form", .before = 2)
