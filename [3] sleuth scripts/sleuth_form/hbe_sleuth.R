...
setwd("/work/giembycz_lab/ALI-HBE-HBE_comparison/sleuth/")
...
s2c_2h <- read_tsv("hbe_meta.txt") %>% mutate(treatment = factor(treatment, levels = c("NS", "Form"))) %>% filter(time == "2h")
s2c_6h <- read_tsv("hbe_meta.txt") %>% mutate(treatment = factor(treatment, levels = c("NS", "Form"))) %>% filter(time == "6h")
...
#save sleuth objects
saveRDS(so_2h, "objects/hbe_2h.rds")
saveRDS(so_6h, "objects/hbe_6h.rds")
