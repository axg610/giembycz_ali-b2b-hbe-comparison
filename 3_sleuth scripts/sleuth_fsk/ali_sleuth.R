...
setwd("/work/giembycz_lab/ALI-B2B-HBE_comparison/sleuth_fsk/")
...
s2c_6h <- read_tsv("ali_meta.txt") %>% mutate(treatment = factor(treatment, levels = c("NS", "Fsk"))) %>% filter(time == "6h")
...
so_6h <- sleuth_wt(so_6h, "treatmentFsk")

#save sleuth objects
saveRDS(so_6h, "objects/ali_6h.rds")

#assemble tpm table

tpm_6h <- kallisto_table(so_6h, use_filtered = FALSE) %>%
  select(Gene = target_id, sample, rep = patient, time, treatment, tpm)

tpm <- tpm_6h

write_tsv(tpm, "results/ali_tpm.txt")

#assemble dea table
dea_6h <- sleuth_results(so_6h, "treatmentFsk", "wt") %>%
  select(Gene = target_id, fold = b, FDR = qval) %>%
  mutate(fold = fold / log(2)) %>%
  mutate(celltype = "ALI", time = "6h", treatment = "Fsk", .before = 2)

dea <- dea_6h

write_tsv(dea, "results/ali_dea.txt")